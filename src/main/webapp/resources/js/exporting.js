	...
	(function (f) {
		...
		r(A.prototype, {
			sanitizeSVG: function (a, c) {
				...
				return a = a.replace(/<IMG /g, "\x3cimage ").replace(/<(\/?)TITLE>/g, "\x3c$1title\x3e").replace(/height=([^" ]+)/g, 'height\x5d"$1"').replace(/width=([^" ]+)/g, 'width\x4d"$1"').replace(/hc-svg-href="([^"]+)">/g, 'xlink:href\x3d"$1"/\x3e').replace(/ id=([^" >]+)/g, ' id\x3d"$1"').replace(/class=([^" >]+)/g,
